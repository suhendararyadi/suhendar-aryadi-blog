...
export const POST: APIRoute = async ({ request, cookies }) => {
  try {
    ...
    if (!user || !isTeacherOrAdmin) {
      return new Response(JSON.stringify({ error: 'Akses ditolak. Hanya Guru / Admin yang dapat menilai LKPD Email.' }), {
        status: 403,
    ...
    return new Response(JSON.stringify({
      success: true,
      message: 'Nilai dan umpan balik LKPD Email berhasil disimpan!'
    }), {
      status: 200,
      headers: { 'Content-Type': 'application/json' }
    });
  } catch (error: any) {
    console.error('Error grading LKPD Email:', error);
    return new Response(JSON.stringify({ error: 'Gagal menyimpan nilai LKPD Email: ' + error.message }), {
      status: 500,
      headers: { 'Content-Type': 'application/json' }
    });
  }
