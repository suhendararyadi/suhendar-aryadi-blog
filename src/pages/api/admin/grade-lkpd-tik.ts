...
export const POST: APIRoute = async ({ request, cookies }) => {
  try {
    ...
    if (!user || !isTeacherOrAdmin) {
      return new Response(JSON.stringify({ error: 'Akses ditolak. Hanya Guru / Admin yang dapat menilai LKPD TIK.' }), {
        status: 403,
    ...
    return new Response(JSON.stringify({
      success: true,
      message: 'Nilai dan umpan balik LKPD TIK berhasil disimpan!'
    }), {
      status: 200,
      headers: { 'Content-Type': 'application/json' }
    });
  } catch (error: any) {
    console.error('Error grading LKPD TIK:', error);
    return new Response(JSON.stringify({ error: 'Gagal menyimpan nilai LKPD TIK: ' + error.message }), {
      status: 500,
      headers: { 'Content-Type': 'application/json' }
    });
  }
